...
/// A running application state pointed at a mock provider, with a credential
/// reference nobody else uses.
///
...
///
/// The credential reference is the field that matters most here, and not for
/// tidiness. `mnema-secrets` keeps the platform store out of reach only under
/// its **own** `cfg(test)` — the `#[cfg(test)]` arm inside `platform_store`,
/// `crates/mnema-secrets/src/lib.rs:307,314` — and an integration test of
/// another crate compiles it without that flag. Everything below therefore
/// writes into the developer's real login keychain. Under the production
/// reference it would overwrite their working key; under a name shared between
/// two fixtures it would cross one test's secret into another. Hence a
/// reference unique per process, per thread and per fixture, and a `Drop` that
/// removes it.
pub struct Fixture {
...
impl Drop for Fixture {
    /// The key is in the operating system's store, not in the temporary
    /// directory, so nothing else would remove it. Not a panic on failure:
    /// panicking in a `Drop` during an unwinding assertion aborts the process
...
fn collect_files(dir: &Path, into: &mut Vec<ScannedFile>) {
    for entry in std::fs::read_dir(dir).expect("the data directory can be listed") {
        ...
        if path.is_dir() {
            ...
            into.push(ScannedFile {
                bytes: std::fs::read(&path).expect("a file in the data directory can be read"),
                path,
            });
        }
    }
