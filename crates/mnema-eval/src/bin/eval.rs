...
use mnema_eval::{
    Corpus, DenseAnswers, IndexedCorpus, QuestionSet, Sweep, corpus_dir, preflight, questions_path,
};
use mnema_search::Provider;

/// The provider is not named, so no content arm can be asked — refusing here
/// is the alternative to silently measuring the lexical arm alone and
/// printing it as though it were the whole sweep.
...
fn main() -> ExitCode {
    // Checked before the worker path: a missing provider is this task's
    // refusal to demonstrate, and it must not hide behind a usage error when
    // neither is given.
    let base = std::env::var("MNEMA_EVAL_BASE");
    let key = std::env::var("MNEMA_EVAL_KEY");
    let (Ok(base), Ok(key)) = (&base, &key) else {
        eprintln!(
            "MNEMA_EVAL_BASE and MNEMA_EVAL_KEY name the live embedding provider \
             this sweep asks — neither is optional:"
        );
        if base.is_err() {
            eprintln!("  missing: MNEMA_EVAL_BASE");
        }
        if key.is_err() {
            eprintln!("  missing: MNEMA_EVAL_KEY");
        }
    ...
    evaluate(Path::new(worker), provider)
}

/// A failure at any step before there is a table to print — loading the
/// corpus or questions, indexing, asking the content arm, sweeping — panics
/// rather than returning a code. These are not conditions this tool handles:
/// they mean the instrument itself is broken, and a panic says so with a
/// message and an exit status (101) the codes above never use.
fn evaluate(worker: &Path, provider: Provider) -> ExitCode {
