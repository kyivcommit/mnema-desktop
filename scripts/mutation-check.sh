...
# in each of its per-file jobs (one per case file in the `mutations` matrix —
# the matrix, not this comment, is the record of how many) to the checkout's
# `target/`, which its cache restores. The Rust legs used to pay a cold build
# each: measured on run 33671866360, when the matrix held six files, 15m11s
# of the step's 24m07s was those builds (3:02, 3:23, 3:02, 3:03, 2:41),
# 2m41s of it in front of one three-second case; the vitest legs compile
# nothing.
