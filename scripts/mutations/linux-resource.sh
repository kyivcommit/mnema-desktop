...
# not-in-matrix: each case breaks ONE platform's bundle config (5 linux, 1 windows, 1 macOS, 2 script); tauri_build::build() reads the target platform's config before any test runs, so the ubuntu-only matrix cannot run the five linux cases — the crate does not compile there. All nine run on macOS (measured red: 9, 2026-09-16); a macOS leg is the same runner-minutes decision as for pr8-exclusions-macos.sh
#
